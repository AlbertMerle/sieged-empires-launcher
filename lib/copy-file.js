...
/** Copy files that are absent at dest; never overwrite existing user files. */
function copyTreeMissingOnly(src, dest) {
  if (!fs.existsSync(src)) return 0;
  fs.mkdirSync(dest, { recursive: true });
  let n = 0;
  for (const name of fs.readdirSync(src)) {
    ...
    const s = path.join(src, name);
    const d = path.join(dest, name);
    if (fs.statSync(s).isDirectory()) n += copyTreeMissingOnly(s, d);
